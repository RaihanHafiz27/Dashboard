...
export const useLogicSettings = () => {
  ...
  const profile = useProfileLogic(profileUser);
  const locations = useLocationLogic(profile.formUser);
  const security = useSecurityLogic(profileUser, profile.formUser);
  ...
  return {
    ...
    profileImage: profile.profilePreview
